...
export default {
  ...
  // 新增对象存储空间要更改数据库版本
  openDB: function (dbname, version, db, newStore,cursorIndex, callback) {
      ...
      request.onupgradeneeded = function (event) {
          ...
          if (newStore) {
            ...
            //   objectStore.createIndex("ownerid", "ownerid", { unique: false });
              if(Object.prototype.toString.call(cursorIndex) === '[object Array]' && cursorIndex.length){
                  for(var i=0;i<cursorIndex.length;i++){
                    objectStore.createIndex(cursorIndex[i], cursorIndex[i], { unique: false });
                  }
              }
          }
      };
  ...
  handleDataByIndex: function (db, storename, cursorIndex, keyRange, callback) {
      var keyRange = keyRange || '';
      var store = db.transaction(storename, 'readwrite').objectStore(storename),
          request;
          console.log('参数',storename, cursorIndex,keyRange)
          if(keyRange){
            request = store.index(cursorIndex).openCursor(keyRange);
          }else{
            request = store.index(cursorIndex).openCursor();
          }
      request.onerror = function () {
